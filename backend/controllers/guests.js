...
export const createGuest = async (req, res, silent = false) => {
  try {
    ...
    if (mediaType === 'video' && body.media) {
      ...
      return res
        .status(201)
        .json({ message: 'Invité (vidéo) ajouté avec succès', guest: doc })
    }
    ...
    return res
      .status(201)
      .json({ message: 'Invité ajouté avec succès', guest: doc })
    /*} catch (error) {
...
export const updateGuest = async (req, res, silent = false) => {
  try {
    ...
    if (!isSilent && res) {
      return res
        .status(500)
        .json({ error: error.message || 'Erreur inconnue dans updateGuest' })
    }
  }
}

...
export const createEdition = async (req, res) => {
  try {
    ...
    uploadedArtistFiles.forEach((f, i) =>
      console.log(`artistFiles[${i}] →`, f.originalname),
    )

    for (const [index, artistData] of (editionData.artists || []).entries()) {
      const needsFile =
        artistData.mediaType !== 'video' && !artistData.mediaFileId
      const mediaFile = needsFile
        ? uploadedArtistFiles[fileCursor++] || null
        : null
    ...
    uploadedGuestFiles.forEach((f, i) =>
      console.log(`guestFiles[${i}] →`, f.originalname),
    )
    ...
    for (const [index, guestData] of (editionData.guests || []).entries()) {
      ...
      const mediaFile = needsFile
        ? uploadedGuestFiles[fileCursor++] || null
        : null
...
export const addArtistToEdition = async (req, res) => {
  try {
    ...
    if (!createdArtist?._id) {
      return res
        .status(400)
        .json({ message: 'Échec de la création de l’artiste' })
    }
...
export const addGuestToEdition = async (req, res) => {
  try {
    ...
    if (!createdGuest?._id) {
      return res
        .status(400)
        .json({ message: 'Échec de la création de l’invité' })
    }
...
export const updateEdition = async (req, res) => {
  ...
  try {
    ...
    const existingEdition = await Edition.findById(
      req.params.editionId || req.params.id,
    )
    if (!existingEdition)
    ...
    for (const [index, artist] of (editionData.artists || []).entries()) {
      ...
      const file = fileIndex !== -1 ? req.files?.artistFiles?.[fileIndex] : null
    ...
    for (const [index, guest] of (editionData.guests || []).entries()) {
      ...
      const file = fileIndex !== -1 ? req.files?.guestFiles?.[fileIndex] : null
    ...
    existingEdition.description =
      editionData.description || existingEdition.description
...
export const deleteEdition = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || 'Erreur serveur (deleteEdition)' })
  }
}

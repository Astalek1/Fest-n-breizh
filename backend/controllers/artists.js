  ...
  try {
    ...
    if (isLogo && isFileId(body.media)) {
      ...
      return res
        .status(201)
        .json({
          message: 'Artiste (logo existant) ajouté avec succès',
          artist: doc,
        })
    }
    ...
    return res
      .status(201)
      .json({ message: 'Artiste ajouté avec succès', artist: doc })
  } catch (error) {
...
export const updateArtist = async (req, res, silent = false) => {
  try {
    ...
    if (!body.mediaType && req.body.mediaType)
      body.mediaType = req.body.mediaType
    ...
    try {
      ...
      console.warn(
        'Erreur pendant la suppression des anciens médias:',
        error.message,
      )
    }
    ...
    if (!isSilent && res) {
      return res
        .status(500)
        .json({ error: error.message || 'Erreur inconnue dans updateArtist' })
    }
  }
}
